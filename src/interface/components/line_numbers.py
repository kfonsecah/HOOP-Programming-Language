...
class LineNumbers(tk.Canvas):
    def __init__(self, master, text_widget, **kwargs):
        ...
        self.configure(
            bg=SEPARATOR_COLOR, # Mismo fondo que el área de texto
    ...
    def redraw(self, *args):
        ...
        # Iterar mientras la línea sea visible
        line_no = int(first_line_index.split('.')[0])
        while True:
            # Obtener las coordenadas del bounding box de la línea
            dline = self.text_widget.dlineinfo(f"{line_no}.0")
            if dline is None:
                break # La línea no es visible, salir del bucle
            
            # Coordenadas y para dibujar el número
            y = dline[1]
            
            # Dibujar el número de línea en el canvas
            self.create_text(
